...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // collapseComments 요소를 부트스트랩의 Collapse 객체로 생성.
    const bsCollape = new bootstrap.Collapse('div#collapseComments', { toggle: false });

    // 댓글 토글 버튼에 클릭 이벤트 리스너를 등록.
    btnToggleComment.addEventListener('click', () => {
        bsCollape.toggle(); // toggle은 열었다 닫았다 할 수 있는 기능
    ...
    function registerComment() {
        ...
        /* const data = {
            postId: postId, // 왼쪽은 data 객체의 프로퍼티 이름, 오른쪽은 registerComment 안쪽에 선언된 지역변수(input에 입력된 postId)
            ctext: ctext,
            username: username
        }; */
        const data = { postId, ctext, username }; // 위와 같음.
        console.log(data);
