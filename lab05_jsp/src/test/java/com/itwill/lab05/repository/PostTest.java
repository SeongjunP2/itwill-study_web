...
//	@Test
	public void testSelect() {
		...
		Assertions.assertEquals(4, result.size()); // List<Post>에 데이터 값이 몇개가 들어있는지 테스트하는 단위 테스트 코드.(JUnit 테스트 결과 확인)
		for (Post p : result) {
...
//	@Test
	public void testInsert() {
		...
		Post post = Post.builder().title("insert테스트").author("테스트author").content("insert Content 테스트").build();
	...
	public void testDelete() {
        ...
        result = dao.delete(0); // id(PK)가 없는 경우
