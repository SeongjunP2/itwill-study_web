...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function registerComment() {
        ...
        // Ajax 요청에서 보낼 데이터
        const data = {postId, ctext, writer}; // postId, ctext, writer 값을 가지고 있는 자바스크립트 객체(Object) 생성
        
        // Ajax POST 방식 요청을 보냄. 응답/에러 처리 콜백 등록.
...
//                if (currentPageNo + 1 < response.data.totalPages) {
                ...
                /*if (currentPageNo === response.data.number - 1) {
                    btnMoreComments.classList.add('d-none');
                }*/
    ...
    function makeCommentElements(data, pageNo) {
        ...
        for (let comment of data) {
            ...
            // 날짜 문자열을 처리하여 초까지만 표시하기
            const formattedDate = formatDate(comment.modifiedTime);
            ...
            <div class="card card-body mt-2">
                ...
                <div class="mt-2">
                    ...
                    `;
                    // 로그인 사용자와 댓글 작성자가 같은 경우에만 버튼을 삭제/업데이트 버튼을 만듦.
        ...
        /*for (let btn of btnDeletes) {
            btn.addEventListener('click', deleteComment);
        }*/ // btnDeletes를 찾아 event를 추가하는 또다른 반복문
    ...
    function updateComment(event) {
        ...
        const textarea = document.querySelector(`textarea.commentText[data-id="${id}"]`); // 업데이트할 댓글 아이디
