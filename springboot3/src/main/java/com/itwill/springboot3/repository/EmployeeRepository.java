...
public interface EmployeeRepository extends JpaRepository<Employee, Integer> {
	...
	// select * from employees where first_name upper(first_name) like upper(?)
	...
	@Query("select e from Employee e where e.department.location.country.countryName = :cName")
	List<Employee> findByCountry(@Param("cName") String country);
	
	// 특정 대륙(예: Europe)에 근무하는 직원들 검색
	@Query("select e from Employee e where e.department.location.country.region.regionName = :region")
	List<Employee> findByRegion(@Param("region") String regionName);
